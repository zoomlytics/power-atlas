...
- [x] **Config-driven**: `run_demo.py ingest-pdf --live` executes `PipelineRunner.from_config_file(...)` against `demo/chain_of_custody/config/pdf_simple_kg_pipeline.yaml` and passes run-scoped `document_metadata`.
...
- Unstructured/PDF ingest remains independent from structured ingest: every run has its own `run_id`; live ingest writes `document_metadata.run_id/source_uri` and then propagates those fields onto all `Chunk` nodes for citation/retrieval provenance.
