...
- **Retrieval is run-scoped by default**: vector search is constrained to `Chunk` nodes matching the active `run_id`. The `ask` command supports `--latest` (default), `--run-id <RUN_ID>`, and `--all-runs` flags to control retrieval scope.

### Manifest layout

| Mode | Manifest path | Key fields |
| --- | --- | --- |
| Batch (`ingest`) | `demo/artifacts/manifest.json` or configured output manifest | `run_id`, `run_scopes.structured_ingest_run_id`, `run_scopes.unstructured_ingest_run_id` |
| Independent stage (`ingest-structured`, `ingest-pdf`) | `demo/artifacts/runs/<run_id>/<stage_name>/manifest.json` | `run_id`, `run_scopes.batch_mode: single_independent_run`, one of `structured_ingest_run_id` / `unstructured_ingest_run_id` |
| Derived stage (`extract-claims`, `resolve-entities`, `ask`) | `demo/artifacts/runs/<run_id>/<stage_name>/manifest.json` | `run_id`, `run_scopes.unstructured_ingest_run_id` or explicit ask scope fields |
