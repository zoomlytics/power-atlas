...
class TestGraphHealthDiagnosticsCliMainArgParsing(unittest.TestCase):
    ...
    def test_parse_args_supports_app_baseline_env_names(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                "APP_NEO4J_URI": "bolt://gh-app.test:7687",
                "APP_NEO4J_USERNAME": "gh-app-user",
                "APP_NEO4J_PASSWORD": "gh-app-secret",
                "APP_NEO4J_DATABASE": "gh-app-db",
            },
            clear=True,
        ):
            args = cli_module._parse_args([], app_baseline=app_baseline)

        self.assertEqual(args.neo4j_uri, "bolt://gh-app.test:7687")
        self.assertEqual(args.neo4j_username, "gh-app-user")
        self.assertEqual(args.neo4j_password, "gh-app-secret")
        self.assertEqual(args.neo4j_database, "gh-app-db")
    ...
    def test_build_request_context_preserves_other_baseline_settings(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "APP_OUTPUT_DIR": "build/graph-health-app",
                "APP_DATASET": "graph_health_dataset",
            },
            clear=True,
        ):
            request_context = build_graph_health_cli_request_context(
                argparse.Namespace(
        ...
        self.assertEqual(request_context.settings.dataset_name, "graph_health_dataset")
