        ...
        node_types=[
            NodeType(
                ...
                description=(
                    ...
                    "**Identity scoping**: ``cluster_id`` encodes ``run_id``, ``source_uri``, "
                    "``entity_type``, and ``normalized_text`` so that clusters are never "
                    "unintentionally merged across processing runs, source documents, or entity "
                    "types. Each component is percent-encoded (RFC 3986) before joining so that "
                    ...
                    "``source_uri=None`` and ``entity_type=None`` are each treated as an empty "
                    "string before encoding, producing an empty segment "
                    "(e.g. ``cluster::run1::::ORG::ibm`` when ``source_uri=None``)."
                ),
                properties=[
