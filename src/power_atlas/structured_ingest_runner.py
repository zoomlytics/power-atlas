...
def resolve_structured_dataset(
    ...
    effective_dataset_id = (
        ...
        else dataset_id_from_fixtures_root(fixtures_dir)
    )
...
def is_blank_csv_row(row: dict[str | None, str | list[str] | None]) -> bool:
...
def lint_and_clean_structured_csvs(
    run_id: str,
    output_dir: Path,
    ...
    *,
    ...
    def add_issue(file_name: str, row_number: int, field: str, code: str, message: str) -> None:
        lint_issues.append(
            {
                "file": file_name,
                "row": row_number,
    ...
    for file_name, expected_headers in resolved_structured_schema.file_headers.items():
            ...
            with source_path.open("r", encoding="utf-8", newline="") as handle:
                ...
                for row_number, raw_row in enumerate(raw_rows, start=2):
                    ...
                    rows.append(
                        (
                            row_number,
                            {header: (raw_row.get(header) or "") for header in expected_headers},
                        )
                    )
        ...
        for row_number, row in deduped:
            ...
            if "value_type" in row:
                ...
                if value_type == "date" and not is_parseable_date((row.get("value") or "").strip()):
                    add_issue(
                        file_name,
                        row_number,
                        "value",
                        "INVALID_DATE_VALUE",
                        "Expected parseable date value",
                    )

            retrieved_at = (row.get("retrieved_at") or "").strip()
            if "retrieved_at" in row and not is_parseable_date(retrieved_at):
                add_issue(
                    file_name,
                    row_number,
                    "retrieved_at",
    ...
    if lint_issues:
        raise ValueError(
            f"Structured CSV lint failed with {len(lint_issues)} issue(s): {lint_report_path}"
        )
    return {
        "run_id": run_id,
        "structured_clean_dir": str(clean_dir),
...
def run_structured_ingest_runtime(
    *,
    ...
    timestamp_factory: Callable[[], str] = timestamp,
    ...
    validation_warnings_path.write_text(
...
def run_structured_ingest_runtime_default(
    *,
    config: object,
    run_id: str,
    fixtures_dir: Path | None = None,
...
__all__ = [
    "dataset_id_from_fixtures_root",
    "deduplicate_rows",
    "is_blank_csv_row",
    "is_parseable_date",
    "lint_and_clean_structured_csvs",
    "load_csv_rows",
    "resolve_structured_dataset",
    "run_structured_ingest_runtime",
    "run_structured_ingest_runtime_default",
    "timestamp",
]