...
**Owner:**  
**Blockers:**  
**Notes:** Phase 10 planning has now started with a concrete shortlist document in `docs/repository_restructure/repository_restructure_phase10_legacy_retirement_shortlist.md`, and two low-risk retirement slices are complete. First, the empty `__queuestorage__/` directory was verified unused and removed after a workspace search found no supported queue-storage or Azure Functions dependency. Second, `_archive/` is now explicitly labeled as archival, non-active material in both `_archive/README.md` and the root `README.md`, while its historical contents remain in place for reference. The latest `demo.contracts` re-inventory also narrowed the shim lane materially: workspace Python-callsite searches still show no non-test runtime imports outside the compatibility layer, simple shim files remain one-line package re-exports, and only the root proxy plus the pipeline alias still require special handling. That makes the simple package-owned `demo/contracts/*.py` re-export shims the clearest next Phase 10 implementation class, while the compatibility shells under `demo/`, `pipelines/query/`, `scripts/`, and `backend/` remain intentionally out of scope for deletion.  
