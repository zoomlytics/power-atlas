...
| Committed run-output archival boundary (`demo/artifacts/`, `pipelines/runs/`) | Phase 10 | Ash | completed | Both roots are still active output locations; current git inventory shows no tracked historical payload under `demo/artifacts/`, while tracked reference exemplars remain under `pipelines/runs/` until runtime defaults and documentation references are intentionally decoupled |
