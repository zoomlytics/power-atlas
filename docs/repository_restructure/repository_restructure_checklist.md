...
**Owner:**  
**Blockers:**  
**Notes:** Phase 10 planning has now started with a concrete shortlist document in `docs/repository_restructure/repository_restructure_phase10_legacy_retirement_shortlist.md`, and the first low-risk retirement slice is complete: the empty `__queuestorage__/` directory was verified unused and removed after a workspace search found no supported queue-storage or Azure Functions dependency. The remaining shortlist now separates `_archive/` as the clearest low-coupling archive candidate from the still-deliberate `demo/contracts` shim-retirement lane. By contrast, the compatibility shells under `demo/`, `pipelines/query/`, `scripts/`, and `backend/` are not yet ready for deletion while those invocation surfaces remain accepted. The practical next implementation slice should therefore be either explicit archive handling for `_archive/` or a deliberate `demo/contracts` shim-retirement class, but not a mixed broad sweep.  
