...
**Owner:**  
**Blockers:**  
**Notes:** Phase 10 now has a resolved shortlist and several completed retirement decisions. First, the empty `__queuestorage__/` directory was verified unused and removed after a workspace search found no supported queue-storage or Azure Functions dependency. Second, `_archive/` is explicitly labeled as archival, non-active material in both `_archive/README.md` and the root `README.md`, and its top-level placement is now an accepted decision: keep it at the repo root as the explicit archival boundary unless a later repository-layout change creates a stronger reason to move it. Third, the `demo.contracts` retirement lane has completed in full: the simple submodule shims were removed in three slices, the root compatibility proxy in `demo/contracts/__init__.py` was retired after rewriting the remaining direct root-import test callers to use package-native imports, the final stateful module-alias shim in `demo/contracts/pipeline.py` was retired after switching logger expectations to the package-native path, and the now-empty `demo/contracts/` directory has been removed. Fourth, the tracked exemplar-retirement follow-up for mixed output roots has now been executed in the working tree: `demo/artifacts/` has no tracked historical payload beyond its ignore control file, and the tracked reference artifacts formerly kept under `pipelines/runs/` have been replaced by durable documentation summaries and deleted from the working tree, leaving only the control file `.gitkeep` there once the deletions are accepted. Fifth, a bounded caller audit of `backend/main.py` has now resolved that shell as an explicit defer-in-place decision rather than an open retirement candidate: it remains the backend container launch seam through `backend/Dockerfile` and the direct import seam for `tests/test_backend_main.py`, so deleting it now would change the active execution posture rather than remove dead compatibility debt. Sixth, the same bounded audit pattern now resolves `scripts/sync_vendor_version.py` as another explicit defer-in-place shell: it remains the stable operator and CI invocation seam in the repo README, vendor docs, and vendor-version-consistency workflow, and it is also the direct import/patch seam exercised by `tests/test_sync_vendor_version.py`. Seventh, the smaller `pipelines/query` shell audit now resolves `pipelines/query/graph_health_diagnostics.py` the same way: it remains the documented manual diagnostics CLI seam and the direct import/patch seam for `demo/tests/test_graph_health_diagnostics_cli.py`, so deleting it now would change the active query CLI/test posture rather than retire dead compatibility debt. Eighth, the heavier companion shell `pipelines/query/retrieval_benchmark.py` resolves the same way after a bounded seam inventory: it remains the documented manual benchmark CLI seam across the demo/query/architecture docs and the direct import/patch seam for `demo/tests/test_retrieval_benchmark_cli.py`, so deleting it now would likewise change the active query CLI/test posture rather than retire dead compatibility debt. Ninth, the follow-up documentation/caller-surface cleanup lane for those accepted defer-in-place shells has now also landed across the root README, operator docs, architecture docs, vendor docs, and secondary reference docs. Tenth, `frontend/` is now also explicitly resolved as a defer-in-place non-core surface for Phase 10: it still participates in the checked-in local scaffold posture through `docker-compose.yml`, the root README, and the narrow `NEXT_PUBLIC_BACKEND_URL` health-check flow in `frontend/app/page.tsx`, so deleting it now would change the accepted placeholder UI posture rather than retire dead compatibility debt. Eleventh, `artifacts/repository_restructure/phase1/` is now also explicitly resolved as a defer-in-place verification-evidence root: it remains the checked-in output location for `make phase1-verify` / `bash scripts/phase1_verify.sh`, and the current checklist, safety harness, plan, and execution log all still anchor accepted Phase 1 proof to that path family, so relocating or deleting it now would change the accepted verification posture rather than retire dead compatibility debt. Twelfth, `docs/repository_restructure/repository_restructure_phase1_execution_run_log.md` is now also explicitly resolved as a defer-in-place verification document: it remains referenced by `scripts/phase1_verify.sh` and still records the accepted run evidence and automation history behind the current Phase 1 posture, so retiring it now would change the accepted verification documentation surface rather than retire dead compatibility debt. Thirteenth, `docs/repository_restructure/repository_restructure_agent_task_breakdown.md` is now also explicitly resolved as a defer-in-place verification/planning-history document: although its own checkpoint note marks it as historical setup context rather than the canonical current plan, the accepted Phase 1 execution run log still references it as part of the authoritative context for interpreting runs, so retiring it now would change the accepted verification documentation surface rather than retire dead compatibility debt. Fourteenth, `docs/repository_restructure/repository_restructure_phase2_demo_contracts_retirement_task.md` is now also explicitly resolved as a defer-in-place historical planning/execution record: its own status and body already mark the underlying `demo/contracts` retirement lane as completed historical work, but the current Phase 10 shortlist and checklist still reference it as the executed follow-up planning task behind that closed lane, so retiring it now would change the accepted restructuring record rather than retire dead compatibility debt. Fifteenth, `studies/SYSTEM-INDEX-v0.1.md` has now been retired as a low-risk stale-doc slice: exact filename searches found no remaining workspace references outside the file itself, and the surviving studies-system docs already treat `/studies/_studies/` plus the workflow/template docs as the canonical inventory and entrypoints. Sixteenth, `_archive/initial_experimentation_2026_02_28/` has now been retired as a low-risk stale archive subtree: nothing outside that dated experimentation subtree still referenced it, its README still documented the old `/examples` demo layout, and removing it did not change the accepted decision to keep `_archive/` itself as the explicit archive boundary. Seventeenth, `demo/artifacts_compare/` is now also explicitly resolved as a defer-in-place manual validation artifact root: no code or automated test callers currently depend on that path family, but the current demo walkthrough, validation runbook, and restructure safety harness still treat it as an accepted comparison-output surface and retained-artifact set for manual validation review, so deleting it now would change the accepted manual validation posture rather than retire dead compatibility debt. Eighteenth, `vendor-resources/tests/` has now been retired as a low-risk stale duplicate vendor subtree: exact path searches found no live workspace references to `vendor-resources/tests`, the retained vendor-reference usage in this repo still points to `vendor-resources/examples`, and removing the duplicate test mirror did not change the accepted vendor-reference posture. Nineteenth, `vendor-resources/docs/` has now been retired as another low-risk stale duplicate vendor subtree: exact path searches found no live workspace references to `vendor-resources/docs`, the subtree mirrored the upstream vendored docs tree, and removing it narrowed the retained vendor-reference posture to `vendor-resources/examples/` without changing live repo behavior. Twentieth, `vendor-resources/images/` has now been retired as another low-risk stale duplicate vendor subtree: exact path searches found no live workspace references to `vendor-resources/images`, the subtree contained only a duplicated upstream image asset, and removing it narrowed the retained vendor-reference posture to `vendor-resources/examples/` without changing live repo behavior. Twenty-first, `demo/fixtures/wikidata_extraction_prompts/` is now explicitly resolved as a defer-in-place operator-facing prototyping template subtree: it is not code-called, but it is still intentionally used by operators during external Wikidata dataset construction in the prototyping phase, so deleting it now would remove retained human workflow templates rather than retire dead compatibility debt. Twenty-second, `docs/governance/relationship-assertion-decision-matrix.md` has now been retired as a low-risk stale governance draft: exact filename and title searches found no live workspace references outside the file itself, and the document described itself as experimental conceptual guidance only, so removing it did not change any accepted execution, verification, or operator workflow surface. Twenty-third, the reset-runtime follow-up slice has now also tightened the structural package boundary: demo graph reset behavior moved under `src/power_atlas/reset_demo_runtime.py`, `demo/reset_demo_db.py` remains only as the operator-facing compatibility seam, and a bounded `src/**/*.py` import audit found no remaining package-to-demo runtime back-edge. Twenty-fourth, the final narrative-extraction caller migration has now retired `demo/narrative_extraction.py`: an exact caller/reference sweep found no remaining non-test code callers or operator-facing docs anchored to that file, the focused `demo/tests/test_narrative_extraction.py` surface moved to `power_atlas.narrative_extraction_cli`, and the focused narrative-extraction test slice remained green after deleting the demo wrapper. The remaining Phase 10 work is now later legacy-surface retirement outside those closed lanes. The compatibility shells under `demo/`, `pipelines/query/`, `scripts/`, and `backend/`, along with the intentionally deferred `frontend/` surface, the active Phase 1 verification artifact root, the active Phase 1 execution run log, the active Phase 1 agent task breakdown, the retained `demo/contracts` retirement-task record, and the retained manual comparison-artifact root under `demo/artifacts_compare/`, remain out of scope for deletion.  
