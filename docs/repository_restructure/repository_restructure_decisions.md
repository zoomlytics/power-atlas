...
- Under `interfaces/api`, `backend/main.py` is now a compatibility shell, but the public backend surface is now `power_atlas.api` rather than the deeper `interfaces` modules. The owning app/runtime assembly lives in `power_atlas.backend_app`, the graph route table lives in `power_atlas.backend_graph_router`, and `src/power_atlas/interfaces/api/*` remains only as a compatibility alias layer over that facade. This preserves the thin-interface intent without treating `interfaces/api` as the long-term package surface.
