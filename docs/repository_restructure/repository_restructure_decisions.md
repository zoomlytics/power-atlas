...
- the current API surface is now owned by `power_atlas.api`; `backend/main.py` imports `create_backend_app` through that package-owned facade, `power_atlas.api` now owns the backend response models, the app-context-backed runtime/bootstrap helpers, a consolidated backend graph query service, the app/router construction helpers, and the initial graph endpoints (`/graph/status`, `/graph/summary`, typed `POST /graph/run-scoped-counts`, and typed `POST /graph/health-summary`), and the deeper `power_atlas.interfaces.api` path remains as compatibility wiring rather than the primary package surface while the FastAPI helpers still avoid becoming root `power_atlas.create_backend_app` / `power_atlas.backend_router` exports,
