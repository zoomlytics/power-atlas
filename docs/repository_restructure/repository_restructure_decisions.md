...
- the current API adapter surface now has a first-class namespace facade at `power_atlas.api`; `backend/main.py` can import `create_backend_app` through that package-owned facade while the underlying FastAPI app/router helpers still avoid becoming root `power_atlas.create_backend_app` / `power_atlas.backend_router` exports,
