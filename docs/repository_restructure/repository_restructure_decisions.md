...
- Under `interfaces/api`, `backend/main.py` is now a compatibility shell, but the public backend surface is now `power_atlas.api` rather than the deeper `interfaces` modules. The owning app/runtime assembly lives in `power_atlas.backend_app`, the graph route table lives in `power_atlas.backend_graph_router`, and the earlier `src/power_atlas/interfaces/api/*` compatibility alias layer has now been retired because there are no remaining callers to protect. This preserves the thin-interface intent without treating `interfaces/api` as the long-term package surface.
...
- keep `power_atlas.api` as the authoritative public backend surface while the
	...
	while `backend/main.py` stays the stable outer seam.
