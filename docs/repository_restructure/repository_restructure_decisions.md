...
- the current API adapter surface under `power_atlas.interfaces.api` follows the same rule: `backend/main.py` should keep importing `create_backend_app` from `power_atlas.interfaces.api`, and the FastAPI app/router helpers should stay under that interface namespace rather than becoming root `power_atlas.create_backend_app` / `power_atlas.backend_router` exports,
