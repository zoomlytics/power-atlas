...
- explicit adapter-local owners now exist for live retrieval session construction plus the `claim_extraction_runtime`, `claim_participation_runtime`, `graph_health`, `retrieval_benchmark`, and `run_scope_queries` modules under `src/power_atlas/adapters/neo4j/`,
- `power_atlas.retrieval_session_setup`, `power_atlas.claim_extraction_runtime`, `power_atlas.claim_participation_runtime`, `power_atlas.graph_health_queries`, `power_atlas.retrieval_benchmark_queries`, and `power_atlas.run_scope_queries` remain as compatibility re-export surfaces so existing stage/test callers do not need to migrate in lockstep,
- the current remaining direct `create_neo4j_driver(...)` owners outside `adapters/neo4j` are `entity_resolution_runtime.py`, `narrative_extraction_runtime.py`, `pdf_ingest_runtime.py`, `retrieval_runtime.py`, and `structured_ingest_runtime.py`,
